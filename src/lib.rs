...
pub fn extract_fixed_sprites(
    ...
    for (entity, visibility, sprite, global_transform, fixed_transform, handle) in
        sprite_query.iter()
    {
        if !visibility.get() {
            continue;
    ...
    ) in atlas_query.iter()
    {
        ...
        if let Some(texture_atlas) = texture_atlases.get(texture_atlas_handle) {
            ...
            let transform = Affine3A {
                matrix3: fixed_transform.affine().matrix3,
                translation: fixed_transform.translation_vec3a()
                    + global_transform.translation_vec3a(),
            }
            .into();
...
impl Plugin for FixedSpritesPlugin {
    fn build(&self, app: &mut App) {
        app.register_type::<FixedSprite>()
            .register_type::<FixedTextureAtlasSprite>()
            .register_type::<FixedTransform>()
            .register_type::<FixedGlobalTransform>()
            .add_systems(
                PostUpdate,
                update_fixed_transforms.in_set(FixedSpriteSystem::UpdateFixedTransforms),
            );
        if let Ok(render_app) = app.get_sub_app_mut(RenderApp) {
            render_app.add_systems(
                ExtractSchedule,
                extract_fixed_sprites.after(SpriteSystem::ExtractSprites),
            );
        }
    }
}
